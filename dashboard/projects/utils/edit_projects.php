...
if ($image_path) {
    $stmt = $db->prepare("UPDATE projects SET title = ?, description = ?, image = ? WHERE id = ?");
...
if ($stmt->execute()) {
    header('Content-Type: application/json');
    echo json_encode(['success' => true, 'message' => 'Project entry updated successfully']);
} else {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Error updating project entry: ' . $stmt->error]);
}
