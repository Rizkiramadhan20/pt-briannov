...
document
  .getElementById("createTestimonialsForm")
  .addEventListener("submit", function (e) {
    ...
    // Send data to server
    fetch("utils/create_testimonials.php", {
      method: "POST",
      body: formData,
    })
      .then((response) => {
      ...
      .then((data) => {
        if (data.success) {
          showToast("Testimonial entry created successfully!", true);
          setTimeout(() => window.location.reload(), 1000);
        } else {
          showToast("Error creating testimonial entry: " + data.message, false);
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("An error occurred while creating testimonial entry", false);
      });
  });
...
document.getElementById("confirmDelete").addEventListener("click", function () {
  ...
  fetch("utils/delete_testimonials.php", {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showToast("Testimonial entry deleted successfully!", true);
        ...
        showToast("Error deleting testimonial entry: " + data.message, false);
      }
    })
    .catch((error) => {
      console.error("Error:", error);
      showToast("An error occurred while deleting testimonial entry", false);
    })
    .finally(() => {
      // Reset loading state
...
document
  .getElementById("editTestimonialsForm")
  .addEventListener("submit", function (e) {
    e.preventDefault();

    const formData = new FormData(this);

    fetch("utils/edit_testimonials.php", {
      method: "POST",
      body: formData,
    })
      .then((response) => {
      ...
      .then((data) => {
        if (data.success) {
          ...
          showToast("Testimonial entry updated successfully!", true);
          setTimeout(() => window.location.reload(), 1000);
        } else {
          showToast("Error updating testimonial entry: " + data.message, false);
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("An error occurred while updating testimonial entry", false);
      });
  });
