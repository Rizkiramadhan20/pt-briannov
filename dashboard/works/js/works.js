...
document
  .getElementById("createWorksForm")
  .addEventListener("submit", function (e) {
    ...
    fetch("utils/create_works.php", {
      ...
      .then((data) => {
        if (data.success) {
          showToast("Work entry created successfully!", true);
          setTimeout(() => window.location.reload(), 1000);
        } else {
          showToast("Error creating work entry: " + data.message, false);
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("An error occurred while creating work entry", false);
      });
  });
  ...
  fetch("utils/delete_works.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showToast("Work entry deleted successfully!", true);
        ...
        showToast("Error deleting work entry: " + data.message, false);
      }
    })
    .catch((error) => {
      console.error("Error:", error);
      showToast("An error occurred while deleting work entry", false);
    })
    .finally(() => {
      // Reset loading state
...
document
  .getElementById("editWorksForm")
  .addEventListener("submit", function (e) {
    e.preventDefault();

    const formData = new FormData(this);

    fetch("utils/edit_works.php", {
      method: "POST",
      body: formData,
    })
      .then((response) => {
      ...
      .then((data) => {
        if (data.success) {
          // Hide modal
          const modal = document.getElementById("editWorksModal");
          const modalInstance = new Modal(modal);
          modalInstance.hide();
          ...
          showToast("Error updating work entry: " + data.message, false);
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        showToast("An error occurred while updating work entry", false);
      });
  });
